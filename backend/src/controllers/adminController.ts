...
export async function getReports(req: Request, res: Response) {
  try {
    ...
    return res.json({
      ...
      byDay: byDayR.rows.map((r) => ({ date: r.date instanceof Date ? r.date.toISOString().split('T')[0] : String(r.date).split('T')[0], orders: parseInt(r.orders), revenue: parseFloat(r.revenue), tips: parseFloat(r.tips) })),
