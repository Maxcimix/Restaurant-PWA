...
const STATUS_ICONS: Record<string, React.ReactNode> = {
  sent_to_kitchen: (<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 11V6a4.5 4.5 0 019 0v5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/><path d="M1 11h11" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/><path d="M6.5 3V1.5M4.5 3.8L3.5 2.8M8.5 3.8L9.5 2.8" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>),
  in_preparation:  (<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.3"/><path d="M6.5 4v2.5l1.5 1.5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>),
  ready_for_pickup:(<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><circle cx="6.5" cy="6.5" r="5" stroke="currentColor" strokeWidth="1.3"/><path d="M4 6.5l2 2 3-3" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>),
  delivered:       (<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M1.5 9.5l2-2 2 2 4-5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round" strokeLinejoin="round"/><path d="M11.5 4.5l-4 5" stroke="currentColor" strokeWidth="1.3" strokeLinecap="round"/></svg>),
  waiting_bill:    (<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><rect x="2" y="1.5" width="9" height="10" rx="1.5" stroke="currentColor" strokeWidth="1.3"/><path d="M4.5 4.5h4M4.5 6.5h4M4.5 8.5h2" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round"/></svg>),
  completed:       (<svg width="13" height="13" viewBox="0 0 13 13" fill="none"><path d="M2 7l3 3 6-6" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/></svg>),
};
...
function MonitorCard({ order, expanded, onToggle }: {
  order: MonitorOrder; expanded: boolean; onToggle: () => void;
}) {
    ...
    <div className={`mc-card ${st.cls}`}>
      <div className="mc-header" onClick={onToggle}>
        <div className="mc-left">
          ...
          {order.tableNumber && <span className="mc-table">Mesa {order.tableNumber}</span>}
        </div>
        <div className="mc-right">
          <span className={`mc-badge ${st.cls}`}>{STATUS_ICONS[order.status]}{st.label}</span>
      ...
      {expanded && (
        <div className="mc-body">
          {order.waiterName && <p className="mc-waiter">{ICON_WAITER}{order.waiterName}</p>}
          ...
          <div className="mc-breakdown">
            <span className="mc-breakdown-row"><span>Subtotal</span><span>{formatCOP(parseFloat(String(order.subtotal)))}</span></span>
            {parseFloat(String(order.tax)) > 0 && (
              <span className="mc-breakdown-row"><span>Impuesto</span><span>{formatCOP(parseFloat(String(order.tax)))}</span></span>
            )}
            {parseFloat(String(order.tip)) > 0 && (
              <span className="mc-breakdown-row"><span>Propina</span><span>{formatCOP(parseFloat(String(order.tip)))}</span></span>
            )}
          </div>
          <div className="mc-footer-row">
            {order.paymentMethod && (
              <span className="mc-method-chip">
                ...
                {{ efectivo:'Efectivo', tarjeta_debito:'Débito', tarjeta_credito:'Crédito', transferencia:'Transferencia', tarjeta:'Tarjeta' }[order.paymentMethod] ?? order.paymentMethod}
              </span>
            )}
            <div className="mc-total">Total: <strong>{formatCOP(parseFloat(String(order.total)))}</strong></div>
          </div>
        </div>
      )}
    </div>
...
export default function CajaMesero() {
  ...
  const [selectedTable,  setSelectedTable]  = useState<WaitingTable | null>(null);
  ...
  return (
    <div className="cajam-root">
      ...
      <div className="cajam-tabs">
        <button type="button" className={`cajam-tab ${activePanel === 'monitor' ? 'cajam-tab--active' : ''}`} onClick={() => setActivePanel('monitor')}>
          ...
          Monitor de órdenes
          {activeMonitor.length > 0 && <span className="cajam-tab-badge">{activeMonitor.length}</span>}
        </button>
        <button type="button" className={`cajam-tab ${activePanel === 'cobro' ? 'cajam-tab--active' : ''}`} onClick={() => setActivePanel('cobro')}>
          <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
          ...
          Cobro de cuentas
          {waitingTables.length > 0 && <span className="cajam-tab-badge cajam-tab-badge--urgent">{waitingTables.length}</span>}
        </button>
      </div>
        ...
        {activePanel === 'cobro' && (
          <div className="cajam-cobro">
            <div className="cajam-search-bar">
              ...
              <input type="search" className="cajam-search"
                placeholder="Buscar por mesa o sección..."
                value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)}/>
            </div>

            {loading ? (
              <div className="cajam-loading"><div className="cashier-spinner"/><p>Cargando mesas...</p></div>
              ...
              <div className="cajam-table">
                <div className="cajam-table-head">
                  <span>Mesa</span><span>Sección</span><span>Mesero</span>
                  <span>Duración</span><span>Método</span>
                ...
                {filteredWaiting.map((table) => (
                  <div key={table.tableId} className="cajam-table-row">
                    ...
                    <span className="cajam-total col-right">{formatCOP(parseFloat(String(table.total)))}</span>
      ...
      {/* Un solo modal — reemplaza BillGenerator + PaymentMethod + TableRelease */}
      {selectedTable && (
        <BillGenerator
          table={selectedTable}
          onPaid={() => { handleClose(); loadTables(); }}
          onClose={handleClose}
        />
      )}
